...
final class SuperLogicaClient {
	...
	<T> SuperLogicaClient withObjectParameter(T t,
		...
		convertValue.entrySet().forEach(entry -> {
			mapParameters.put(upperCaseParameterKeys
					? entry.getKey().toUpperCase() : entry.getKey(),
	...
	private ClientResponse makeRequest() {
		...
		WebResource webResource = buildClient()
				.resource(apiVersion.getFullUrl(endpoint));
		...
		return webResource
				...
				.method(httpMethod, ClientResponse.class, mapParameters);
	}
